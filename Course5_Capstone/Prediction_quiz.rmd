...
pred <- exp(predict.lm(model.AIC, ames_test))
mean(pred)
# Answer: 177220.3
```
...
sqrt(mean((ames_test$price - pred) ** 2))
...
pred_BIC <- exp(predict(model.BIC, ames_test, interval = "prediction"))
mean(ames_test$price > pred_BIC[,"lwr"] & ames_test$price < pred_BIC[,"upr"])
...
pred.test.MPM <- predict(model.bas, ames_test, estimator = "MPM", prediction=TRUE, se.fit = TRUE)
