...
m_logprice_full <- lm(log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train)
log_n <- log(nrow(ames_train))

# AIC
stepAIC(m_logprice_full, k = 2)

# BIC
stepAIC(m_logprice_full, k = log_n)
...
m_simple <- lm(log(price) ~ log(area), data=ames_train)

ames_train %>% mutate(resid = residuals(m_simple)) %>%
...
ames_train %>% mutate(resid = residuals(m_simple) ** 2) %>%
  group_by(Neighborhood) %>%
  summarise(Mean = mean(resid)) %>%
  arrange(desc(Mean))
...
sort(sapply(ames_train %>%
              select(price, Overall.Qual, Bsmt.Qual, Garage.Qual), function(y) sum(length(which(is.na(y))))), decreasing= TRUE)
...
m_overall <- lm(log(price) ~ Overall.Cond + Overall.Qual, data=ames_train)
pred <- predict(m_overall)
ames_train %>% mutate(pred = pred) %>%
...
m_simple_filtered <- lm(log(price) ~ log(area), data = ames_train %>%
  filter(Sale.Condition == "Normal"))
summary(m_simple_filtered)
