...
bayes_inference(y = weight, data = nc_fullterm, type = "ht", statistic = "mean", method = "theoretical", alternative = "twosided", mu_0 = 7.7, cred_level = 0.95, show_plot = FALSE, rscale = 1, prior_family = "JZS")
```
...
nc_premie <- nc %>% filter(premie == 'premie') 

weight_premie_post <- bayes_inference(y=weight, data=nc_premie, type="ci", statistic = "mean", method = "simulation", mu_0 = 7.7, rscale = 1, show_plot = FALSE, prior_family = "JZS", cred_level = 0.95)

samples_premie <- as.data.frame(weight_premie_post$samples)
nrow_premie <- nrow(samples_premie)
samples_premie <- samples_premie %>% mutate(y_pred = rnorm(nrow_premie, mu, sqrt(sig2)))

samples_premie %>% select(mu, y_pred) %>%
...
ggplot(data=nc_fullterm, aes(x=habit, y=weight)) +
  geom_boxplot()
```
...
# Type your code for the question here.
bayes_inference(y=weight, x=habit, data=nc_fullterm,
                statistic = "mean", method = "theoretical", type = "ht",
                null = 0, rscale = 1, hypothesis_prior = c(0.25, 0.75), alternative = "twosided",
...
bayes_inference(y=weight, x=habit, data=nc_fullterm,
                ...
                prior_family = "JZS", null=0, rscale = 1 / sqrt(2), alternative = "twosided",
                show_plot = FALSE)
```
...
bayes_inference(y=weight, x=habit, data=nc_fullterm,
                type = "ci", statistic = "mean", method = "theoretical",
                prior_family = "JZS", mu_0 = 0, rscale = 1 / sqrt(2), cred_level = 0.95,
                show_plot = FALSE)
```
...
pss_sport <- PrisonStress %>% filter(Group == "Sport") %>% mutate(diff=PSSafter - PSSbefore)

bayes_inference(y=diff, data = pss_sport, 
                type = "ht", method = "simulation", statistic = "mean",
                prior_family = "JZS", null = 0, rscale = 1, alternative = "twosided",
                show_plot = FALSE)
```
