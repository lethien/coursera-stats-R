...
tidy(m_wage_edu)
ggplot(data = wage, aes(wage, educ)) +
  geom_point() +
  stat_smooth(method = 'lm', se = FALSE)
confint(m_wage_edu)
```
...
m_wage_edu_aug <- augment(m_wage_edu)

ggplot(data = m_wage_edu_aug, aes(.fitted, .resid)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  labs(x = 'Fitted Values', y ='Residuals')

ggplot(data = m_wage_edu_aug, aes(.resid)) +
  geom_histogram(binwidth = 100) +
  xlab('Residuals')

ggplot(data = m_wage_edu_aug) +
  geom_qq(aes(sample = .std.resid)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed') +
  labs(x = 'Theoretical quantiles', y = 'Standardized residuals')
```
...
ggplot(data = m_lwage_iq_aug, aes(.resid)) +
  geom_histogram() +
  xlab('Residuals')

ggplot(data = m_lwage_iq_aug) +
  geom_qq(aes(sample = .std.resid)) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed') +
  labs(x = 'Theoretical quantile', y = 'Residuals')
```
...
log_n <- log(nrow(na.omit(wage)))
stepAIC(m_lwage_full, k = log_n)
```
