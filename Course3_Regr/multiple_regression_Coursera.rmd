...
```{r summary-model}
# type your code for the Exercise here, and Knit
lm <- lm(score ~ bty_avg, data=evals)
summary(lm)
```
...
ggplot(data = lm, aes(x = lm$fitted.values, y= lm$residuals)) +
  geom_point() +
  geom_hline(yintercept = 0)

ggplot(data= lm, aes(x=lm$residuals)) +
  geom_histogram()
```
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
ggplot(data=m_bty_gen, aes(x=m_bty_gen$fitted.values, y=m_bty_gen$residuals)) +
  geom_point() +
  geom_hline(yintercept = 0)

ggplot(data=m_bty_gen, aes(x=m_bty_gen$residuals)) +
  geom_histogram()
```
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_drop_cls_profs <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_drop_cls_profs)
```
